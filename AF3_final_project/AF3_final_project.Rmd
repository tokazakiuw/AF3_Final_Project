...
```{r introduction-stats, echo=FALSE, include=FALSE}
...
We can see with 95% confidence, the difference in rates between whites and Non-Hispanic African Americans falls in between 10 \~ 12 people higher for Non-Hispanic Black Individuals and the difference between white and Hispanic people is 3 \~ 5 people higher. Furthermore, using the p-value of 0 found from both differences in means, we reject the null and accept the alternative hypothesis stating that being a Hispanic or Non-Hispanic Black individual does increase the risk of being hospitalized for Cardiovascular Disease in the US.
...
$$Hospitalizations\ of\ Non-Hispanic\ Black\ Individuals\ in\ West\ Coast = 33.979\ -\ 10.9035\ *\ Non-Hispanic\ White\ -\ 7.893033\ *\ in\_west$$ $$Hospitalizations\ of\ Non-Hispanic\ Black\ Individuals\ in\ South = 31.030483 \ -\ -10.916\ *\ Non-Hispanic\ White\ +\ 4.195183   \ *\ in\_west$$

$$Hospitalizations\ of\ Hispanics\ in\ West\ Coast = 27.273 - 4.257\ *\ Non-Hispanic\ White\ -\ 7.619\ *\ in\_west$$ $$Hospitalizations\ of\ Hispanics\ in\ South = 24.558 + Non-Hispanic\ White -4.218 + in\_south$$ From these models, we see that around 31 \~ 33 Non-Hispanic Black Individuals and around 24 \~ 27 Hispanics are expected to be hospitalized for cardiovascular disease. The difference of \~10 people in the rate of African Americans compared to White Americans and the difference of \~4 people of Hispanics compared to White Americans is also consistent with the values found in our 95% confidence interval. It also shows that in more diverse, liberal places such as the west coast, the rate of hospitalization goes down by 7 \~ 8 people for Hispanics and African Americans and goes up by around 3 \~ 4 in the more conservative south.

#### RMSE for the Models of Hospitalization of African Americans and Hispanics
...
At the beginning of this research question we inquired about the relationship between race and cardiovascular hospitalizations in the US. Looking at the average rates, medians, and visuals of Hispanics and Non-Hispanic Black Individuals compared to Non-Hispanic Whites, it is clear these races suffer from a higher rate of cardiovascular hospitalizations. At a 95% confidence level, we found that a higher number of Hispanics and a significantly larger amount of African Americans are hospitalized for cardiovascular issues compared to Caucasians. With a p-value of 0, the differences between each group was significant, meaning that we did reject a null and found that being Hispanic or Black has a relationship with higher rates of hospitalization. Furthermore, attempting to predict these numbers with linear models might be possible, however, within this data set there are not enough statistics of the possible variables involved that would help us create that sort of predictor.
...
age_comparison <- cms_data %>% 
  ...
  filter(Data_Value_Unit == "Percent (%)")
```
...
Looking at the mean and median values for each disease topic and two age groups, the difference between demographics and outliers are minimal in this cardiovascular disease data set. However, in the age 65+ group with Major Cardiovascular Diseases, there was the greatest standard deviation and IQR among other diseases in all age group. Therefore, the observations for this sample is more spread out compared to other diseases (eg. Stroke).
...
Linear effects model for age and prevalence (%) of cardiovascular disease among hospitalizations:

*Prevalence (%) of Cardiovascular Heart Disease among Hospitalizations= 2.634 + Coronary Heart Disease \* 3.906 + Diseases of the Heart \* 15.069 + Heart Failure \* 2.067 + Major Cardiovascular Disease \* 20.930 + Stroke \* 0.885 + Age 75+ \* 0.488*

The baseline for this model is Acute Myocardial Infarction (Heart Attack) and age 65+, and this model means, when there is no other diseases except for Acute Myocardial Infarction, the percentage of people from age group of 65+, will be about 2.634. When Coronary Heart Disease is true (value of 1), the percentage will add by 3.906 on top of 2.634 baseline value. If Disease of the Heart is true, the percentage will add 15.069 on top of 2.634. If Heart Failure is true, the percentage will add 2.067 on top of 2.634. If Major Cardiovascular Disease is true, the percentage will add 20.930 on top of 2.634. If Stroke is true, the percentage will add 0.885 on top of 2.634. Lastly, if the observed patient is in the age group of 75+, the percentage will add 0.488 on top of 2.634; although it is a minor change, being in the 75+ age group increases the likelihood of cardiovascular disease prevalence for all hospitalizations by 0.488 percent on average.

```{r spliting-data, echo=FALSE, message=FALSE}
## Set seed to 8239
set.seed(8239)
...
model1_age <- lm(Data_Value ~  Age + Topic, data=age_train)
...
## Examine specific Variable for Insight on Distribution of Residuals
## ggplot(data=model1_age_aug %>% 
##         filter(Age=="65+", Topic=="Stroke"), 
##       mapping=aes(x=.fitted,
##                   y=.resid)) +
##  geom_point() +
##  geom_hline(yintercept=0,color="red")
...
model1_age_pred_rmse <- rmse(model1_age_pred, truth=Data_Value, estimate=pred)
...
Splitting the age_comparison data into a training set with the size of 70% and a test set with the size of 30%, we get marginally different computations in comparison to the original linear effects model. The adjusted r squared for this training model is `r train_rsquared`, in this case, about 96.8% of the data from age and cardiovascular heart diseases is explained by the linear model. Furthermore, the predicted rmse value for the split data frame is `r model1_age_pred_rmse`, which is relatively low and illustrates that age and topics are good predictors for percent composition of cardiovascular disease prevalence among hospitalizations.

Since the adjusted r squared of the linear effects model shows that our linear model is significant, we can tell that different types of cardiovascular diseases have varying impacts on the prevalence of cardiovascular disease among hospitalized patients. Comparing with all the diseases, however, the impact of age demographics (65+ vs. 75+) on the percentage of people having cardiovascular diseases will be relatively insignificant compared to others. Even though it shows that there isn't a strong relationship between age and cardiovascular disease prevalence, there are not enough samples from other age groups to tell us the true impacts of age on cardiovascular disease prevalence. So from this data set, it isn't sufficient for us to get to a conclusion if there is an actual relationship between age and prevalence of cardiovascular diseases among hospitalizations.

## Conclusion

From the collective statistical analyses of our proposed research questions, our understanding of the relationship between predictor variables and susceptibility for cardiovascular disease has furthered. Whether it was rejecting null hypotheses through testing statistical significance or observing the numerical effects of predictors, we have a greater understanding of the influences a variable can have on the underlying issue of cardiovascular disease hospitalizations. In terms of critiques and suggestions, a consensus was made that our data set was not representative of the larger U.S. population and subsequent statistical conclusions were not concrete. Furthermore, we agreed that cross-referencing data sets would lead to increased accuracy of our findings and provide further justifications for relationships between predictor variables and cardiovascular disease related hospitalizations. If analysis on this issue were to be conducted again, it is essential to incorporate diverse data sets with differing demographics and variables to truly gather insights on this critical issue.
