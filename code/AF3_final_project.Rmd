...
location_data <- cms_data %>%
  filter(Break_Out_Category == "Race", Data_Value_Unit == "Rate per 1,000") %>%
  filter(!is.na(Break_Out), !is.na(Data_Value)) %>%
  #group_by(Break_Out) %>%
  count(LocationAbbr, Break_Out) 
...
**Null Hypothesis**: Gender has no influence on a patient's susceptibility for high rates of cardiovascular disease related hospitalizations. The difference in proportion for high rates of heart failure related hospitalizations between Males and Females equals zero.

**Alternative Hypothesis**: Gender will influence a patient's susceptibility for high rates of cardiovascular disease related hospitalizations. Specifically, Males will be adversely impacted by high rates of cardiovascular disease related hospitalizations in proportion to Females. The difference in proportion in high rates of heart failure related hospitalizations between Males and Females is greater than 0.

```{r gender-statistical-analysis-setup, echo=FALSE}
## Filter data frame by Gender and high rates of heart failure hospitalizations
gender_high_rates <- cms_data %>% 
  filter(Break_Out_Category == "Gender", Data_Value_Unit=="Rate per 1,000") %>% 
  filter(!is.na(Break_Out), !is.na(Data_Value)) %>% 
  filter(Data_Value > 23)

## Count observations in filtered data frame
gender_high_rates %>% 
  count()

## Assign null difference in proportion
obs_diff <- 0
```

In this analysis, statistical methods and \_\_
...
## Calculate 1000 different resamples of data and calculate the difference in mean value between Males and Females
gender_high_boot_diff_prop <- cms_data %>%
  filter(Break_Out_Category == "Gender", Data_Value_Unit=="Rate per 1,000") %>% 
  filter(!is.na(Break_Out), !is.na(Data_Value)) %>% 
  specify(Data_Value ~ Break_Out, success="Male") %>%
...
## Give the lower and upper bounds of a 95% bootstrap confidence interval for gender_boot_diff_mean
gender_high_ci <- gender_boot_diff_mean %>% 
  summarize(mean=mean(stat),
            lower=quantile(stat, 0.025),
            upper=quantile(stat, 0.975))
gender_high_ci
...
gender_high_boot_diff_prop %>% 
  visualize()+
  labs(title="1000 bootstraps of the difference in proportions",
       subtitle="95% confidence interval",
       y="count",
       x="Bootstrapped difference in proportion between Males and Females for high rates of heart failure hospitalizations") +
  shade_p_value(obs_stat=0, direction="both")
```
...
diseases_label <- c("Heart Attack", "Coronary Heart Disease", "Heart Disease", 
...
ggplot(data = cms_data %>% 
         filter(!is.na(Data_Value), Break_Out_Category == "Age") %>% 
         rename(Age = "Break_Out"),
...
From the graphs, we can see that there is a relationship between age and heart diseases, for most type of heart diseases displayed in the graphs, the 75+ age group have a higher percentage of having different types of heart diseases except for coronary heart diseases.
