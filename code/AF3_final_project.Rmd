...
get_confidence_interval(black_boot_diff_mean)
```
...
get_confidence_interval(hisp_boot_diff_mean)
```
...
From these models, we see that  around 31 ~ 33 Non-Hispanic Black Individuals and around 24 ~ 27 Hispanics are expected to be hospitalized for cardiovascular disease.
...
black_pred <- predict(black_fit, newdata=black_test) %>%
  bind_cols(black_test %>% select(Data_Value)) %>%
  rename(pred = ...1)

black_pred

rmse(black_pred, truth = Data_Value, estimate = pred)
```
...
In this analysis, [explain the re coding of data and new High_Rate variable, foundation to statistical analysis, and justify using statistical methods (what is represents, why using it, and compare to alternatives),
...
Here is an starting data summary of the data set comparing the relationship between age and different types of heart diseases
