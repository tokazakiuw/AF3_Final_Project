...
```{r diff_in_means_black, echo=FALSE, include=FALSE}
set.seed(5734)
...
```{r black-boot, echo=FALSE, message=FALSE, warning=FALSE}
...
black_boot_diff_mean %>%
   get_p_value(obs_stat=0.05, direction="both")
...
```{r hisp-boot, echo=FALSE, message=FALSE, warning=FALSE}  
...
```{r black-race-modeling, echo=FALSE}
...
# create regression model of hospitalizations in west coast
black_model_west <- lm(Data_Value ~ Break_Out + in_west, data = black_only)
tidy(black_model_west)

# create regression model of hospitalizations in south
