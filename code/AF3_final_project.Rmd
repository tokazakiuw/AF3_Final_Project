...
Cardiovascular disease and Hospitalization is a crucial topic because it is so closely related with our lives without our notice, from other statistical data provided by the [*Centers for Disease Control and Prevention (CDC)*](https://www.cdc.gov/heartdisease/facts.htm), it shows that **one person dies every 36 seconds** in the United States from cardiovascular disease. About 659,000 people in United States die from heart diseases, which is about 1 in every four deaths. Treatments for cardiovascular diseases of people is also incredibly expensive and it could take a long time for it to be cured or, in many cases, it may never be cured. It costs the United States about \$363 billion each year from 2016 to 2017. This includes the cost of health care services, medicines, and lost productivity due to death. In addition, since cardiovascular diseases is so dangerously common, it interests us to discover more about cardiovascular disease's relationship between variables, such as location, age, race, and gender.
...
west_coast <- avg_mean %>%
  filter(LocationAbbr %in% c("CO", "ID", "MT", "NV", "UT", "WY", "AK", "CA", "HI", "OR", "WA"))
west_coast

south_states <- avg_mean %>%
  filter(LocationAbbr %in% c("MD", "DE", "VA", "WV", "KY", "TN", "NC", "SC", "FL", "GA", "AL", "MS", "LA", "AR", "TX", "OK"))
south_states

black_data <- avg_mean %>%
  filter(Break_Out == "Non-Hispanic Black") %>%
  mutate(higher_rate = case_when(mean > (white_mean + black_diff) ~ 1, TRUE ~ 0)) %>%
  mutate(lower_rate = case_when(mean < (white_mean + black_diff) ~ 1, TRUE ~ 0))

hisp_data <- avg_mean %>%
  filter(Break_Out == "Hispanic") %>%
  mutate(higher_rate = case_when(mean > (white_mean + hisp_diff) ~ 1, TRUE ~ 0)) %>%
  mutate(lower_rate = case_when(mean < (white_mean + hisp_diff) ~ 1, TRUE ~ 0))
...
[explain the re coding of data and new High_Rate variable, foundation to statistical analysis, and justify using statistical methods (what is represents, why using it, and compare to alternatives)]

In this analysis, a bootstrap difference in proportions can be used to gain insights on the null/alternative hypothesis. Setting the infrastructure for analysis, a new column (variable) called `High_Rate` can be created which categorizes observations by a boolean expression which denotes whether the rate of heart failure observed was high or not ("yes"/"no"). `High_Rate` specifically describes whether an observation had a heart failure rate greater than or equal to 50 for gender observations (if `Data_Value` \>= 50, `High_Rate` == "yes").
...
cms_hf <- cms_filter %>% filter(Topic == "Heart Failure", Data_Value < 100, Region == "Rocky Mountain" | Region == "Southeast")
ggplot(data = cms_hf,
...
(I will add inference and conclusion soon!)
...
## Set up a linear model to compare relationship
